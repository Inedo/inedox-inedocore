...
public sealed class ADUserDirectoryV4 : UserDirectory
{
    ...
    public ADUserDirectoryV4()
    {
        ...
        this.localTrusts = OperatingSystem.IsWindows() ? new Lazy<HashSet<string>>(this.GetCurrentDomainAndTrusts) : new Lazy<HashSet<string>>([]);
    }
    ...
    ****************************************************************************************************/
    ...
    private PrincipalId CreatePrincipleId(LdapClientEntry result)
    {
        ...
        var isUser = objectCategory.Contains("CN=Person", StringComparison.OrdinalIgnoreCase);
        var isGmsa = objectCategory.Contains("CN=ms-DS-Group-Managed-Service-Account", StringComparison.OrdinalIgnoreCase);
