...
[DisplayName("V3: Active Directory (Deprecated)")]
...
public sealed class ADUserDirectory : UserDirectory
{
    ...
    public override async IAsyncEnumerable<IUserDirectoryUser> GetGroupMembersAsync(string groupName, [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        var group = (ActiveDirectoryGroup)await this.TryGetGroupAsync(groupName);
    ...
    public override async Task<IUserDirectoryUser> TryGetAndValidateUserAsync(string userName, string password, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            await conn.ConnectAsync(AH.NullIf(this.DomainControllerAddress, string.Empty), int.TryParse(this.Port, out var port) ? port : null, this.UseLdaps, this.BypassLdapsCertificateValidation);
    ...
    public async ValueTask<IUserDirectoryUser> TryParseLogonUserAsync(string logonUser)
    {
        ...
        return await this.TryGetUserAsync(domainLogin);
    }
    ...
    private async Task<LdapClientEntry> TryGetPrincipalAsync(PrincipalSearchType searchType, string principalName)
    {
        ...
        foreach (var domain in domains)
        {
            ...
            //BASE DN
            var result = await this.SearchAsync("DC=" + domain.Name.Replace(".", ",DC="), searchString.ToString(), userName: domain.DomainQualifiedName, password: domain.Password).FirstOrDefaultAsync();
            if (result != null)
                return result;
        }
    ...
    private async IAsyncEnumerable<IUserDirectoryPrincipal> FindPrincipalsUsingLdapAsync(PrincipalSearchType searchType, string ldapSearch = null, LdapClientSearchScope scope = LdapClientSearchScope.Subtree)//, ReferralChasingOption? referralChasingOption = null)
    {
        var userSearchQuery = "objectCategory=user";
        if (this.IncludeGroupManagedServiceAccounts)
            userSearchQuery = "|(objectCategory=user)(objectCategory=msDS-GroupManagedServiceAccount)";
        ...
        foreach (var domain in this.domainsToSearch.Value)
        {
            this.LogDebug("Searching domain: " + domain);
            //BASE DN
            await foreach (var result in this.SearchAsync("DC=" + domain.Name.Replace(".", ",DC="), filter, scope: scope, userName: domain.DomainQualifiedName, password: domain.Password))
            {
    ...
    private sealed class ActiveDirectoryUser : IUserDirectoryUser, IEquatable<ActiveDirectoryUser>
    {
        ...
        public ActiveDirectoryUser(ADUserDirectory directory, UserId userId, string displayName, string emailAddress, HashSet<string> groupNames = null)
        {
    ...
    private sealed class ActiveDirectoryGroup : IUserDirectoryGroup, IEquatable<ActiveDirectoryGroup>
    {
        ...
        public ActiveDirectoryGroup(ADUserDirectory directory, GroupId groupId, HashSet<string> groupNames = null)
        {
            this.directory = directory;
        ...
        internal async IAsyncEnumerable<IUserDirectoryUser> GetMembersAsync()
        {
            ...
            var users = this.directory.FindPrincipalsUsingLdapAsync(PrincipalSearchType.UsersAndGroups, $"(memberOf={groupSearch.GetPropertyValue("distinguishedName")})", LdapClientSearchScope.Subtree);
