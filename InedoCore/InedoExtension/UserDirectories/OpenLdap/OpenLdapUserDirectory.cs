...
public sealed partial class OpenLdapUserDirectory : UserDirectory
{
    ...
    ****************************************************************************************************/
    ...
    private async Task<HashSet<string>> GetGroupNamesAsync(PrincipalId principalId)
    {
        ...
        var groupFilter = this.UserGroupsFilter.Replace("%s", OperatingSystem.IsLinux() ? LdapHelperV4.EscapeFilterValue(principalId.DistinguishedName) : principalId.DistinguishedName);
    ...
    {
        using var ldapClient = await this.GetClientAndConnectAsync(true);
        var memberFilter = this.GroupMembersFilter.Replace("%s", OperatingSystem.IsLinux() ? LdapHelperV4.EscapeFilterValue(principalId.DistinguishedName) : principalId.DistinguishedName);
